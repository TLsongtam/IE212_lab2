...
words_cat = FOREACH final_sorted GENERATE category, FLATTEN(TOKENIZE(clean_review)) AS word;
group_cat = GROUP words_cat BY (category, word);
word_count_cat = FOREACH group_cat GENERATE 
    FLATTEN(group) AS (category, word), 
    COUNT(words_cat) AS freq;
group_all_cat = GROUP word_count_cat BY category;
top5_cat = FOREACH group_all_cat GENERATE 
    group AS category, 
    TOP(5, 2, word_count_cat);
STORE top5_cat INTO 'file:///D:/VisualStudio/IE212/lab2/output_bai5' USING PigStorage(';');